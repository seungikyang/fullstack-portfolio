...
아래 표는 단계마다 근거를 잃지 않기 위한 고정 기록 위치입니다. 긴 로그는 별도 파일에 두고 이 표에는 파일 경로나 핵심 한 줄만 적어도 됩니다.

| 단계 | 실행 명령·경로 | 관찰한 정상·실패 결과 | 해결한 오류 | 직접 바꾼 코드 위치 | 면접 한 문장 |
| ---- | -------------- | --------------------- | ----------- | ------------------- | ------------ |
| 01   |                |                       |             |                     |              |
| 02   |                |                       |             |                     |              |
| 03   |                |                       |             |                     |              |
...
`npm run progress`는 지정된 starter 소스의 전용 `____` 토큰과 체크리스트 전체 현황을 보여줍니다. `npm run verify:structure`는 저장소 구조를 검사하고, `npm run verify:learning`은 전용 토큰이 0개이면서 체크리스트 106개가 모두 완료됐을 때만 통과합니다. 일반 `TODO`와 서술형 답안의 품질은 자동 판정하지 않으므로 위 표와 실제 실행 결과를 함께 확인합니다.
