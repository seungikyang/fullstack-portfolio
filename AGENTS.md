...
| 2026-07-27 | 폴더명 기반 Orca 고정 URI 추가          | 루트 문제집 서버·테스트·`README.md`·`history.html`                                                                                     | 상위 폴더 노출 없이 `fullstack` URI를 고정하고 Orca 공식 CLI 실행을 같은 주소로 재현          |
| 2026-07-28 | 문제집 UI·문서 전환 UX 검증             | 루트 목차·Markdown 렌더링 서버·앵커·테스트·실행 문서                                                                                   | 모바일 고정 헤더 가림을 해소하고 문제→힌트→정답을 안전한 웹 문서로 연결                      |
