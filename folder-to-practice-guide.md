...
순서대로 진행하세요. 마감 트랙(15~17)만 보면 자기 코드 근거가 없어 답변이 외운 티가 나고, 보강 트랙(9~14)만 보면 면접 통과율이 떨어집니다.
...
| 11 | `11-java-spring` | Spring Boot 게시판 API | 계층 분리, 의존성 주입, JPA | "대형 SI의 Spring 프로젝트에 합류 가능" | `./gradlew bootRun` 동작 영상 |
...
| 17 | `17-interview-prep` | 자기소개·기술·인성 답안 카드 | 자기 코드 근거 답변, STAR 기법 | "기술 + 인성 면접을 모두 통과한다" | 30초·1분·3분 자기소개 녹음 |
