...
선택된 에이전트들을 **단일 메시지에서 병렬로** 호출한다. `TeamCreate`로 팀을 구성하고 `TaskCreate`로 각 에이전트에게 작업을 배정한다.
...
메인 Claude가 `_workspace/04_priority_report.md`를 사용자에게 보여주고 묻는다:
...
3. CLAUDE.md의 변경 이력 테이블에 한 줄 추가 (날짜, 변경, 대상, 사유)
...
- 평균 작업/팀원: 4~6개
- 통신: TaskCreate(작업 추적) + 파일(`_workspace/`) + SendMessage(즉시 조율)
...
9. Phase 7: 사용자 피드백 수집, CLAUDE.md 변경 이력에 한 줄 기록
