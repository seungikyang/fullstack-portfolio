...
| 좋은 진술 | 나쁜 진술 |
|----------|----------|
| "JWT 만료 1시간, bcrypt 라운드 10" | "안전하게 인증을 구현" |
...
- JWT 만료: `server/auth.js`의 `signToken({ expiresIn: '1h' })`
```
...
> "Express + React 풀스택 Career Hub (JWT 인증, CRUD, Vitest 30+, Docker, GitHub Actions)"
...
> Vitest 단위·통합 테스트 30+건과 supertest API 테스트, smoke test를 작성했으며, GitHub Actions에서 lint→test→build→docker 게이트를 자동 검증합니다.
> 멀티 스테이지 Dockerfile과 fly.io/Render 배포 매니페스트로 실배포까지 경험했습니다.
...
```markdown
- [ ] `npm run verify` 통과 — CI와 동일한 게이트
- [ ] `npm run clean:generated` 후 ZIP 압축 (node_modules 제외)
- [ ] README 데모 계정으로 실제 로그인 가능
```
