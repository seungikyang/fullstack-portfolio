...
예시:
> (S) 로그인 토큰이 무제한이면 탈취 시 장기간 접근될 수 있습니다. (T) 만료 범위를 정하고 검증해야 했습니다. (A) `server/auth.js`의 `TOKEN_EXPIRES_IN`을 `2h`로 두고 `requireAuth`에서 HS256 토큰을 검증했습니다. (R) 발급 토큰의 유효 시간을 2시간으로 제한했고 관련 테스트로 설정을 확인했습니다.

### 3. Node로 만든 것을 Java/Spring 어휘로 다리 놓기

Java/Spring 공고의 면접에서는 Spring 구조와의 차이를 물을 수 있습니다. Node로 작성한 범위와 Spring 학습 범위를 구분하고, 관련 질문에만 **다리(bridge) 문장**을 붙입니다.
...
### 좋은 문장 (정량 + STAR 압축)
> "Express REST API와 React 화면을 연결하고 `server/auth.js`의 JWT·bcrypt 인증, helmet CSP 설정을 직접 확인했습니다. 최신 `npm run verify`와 GitHub Actions 로그가 있을 때 format·lint·test·build·docker 검증 결과를 함께 설명합니다."
