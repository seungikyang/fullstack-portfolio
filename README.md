...
코드형 문제의 `problems.md` 상단에는 실제 실습 파일 링크가 있습니다. `?view=source`가 붙은 링크는 HTML·JavaScript·TypeScript·SQL·Docker 설정을 실행하지 않고 웹 코드 편집기로 표시합니다. 코드를 수정한 뒤 `파일 저장`을 누르면 같은 로컬 파일에 반영되며, 같은 화면에서 1~3단계 힌트와 정답 비교를 선택해 볼 수 있습니다. 웹 저장은 문제에 연결된 실습 파일 50개에만 허용되고, 외부 변경이 감지되면 다시 불러오기 전까지 덮어쓰지 않습니다. 11단계는 Spring 프로젝트 생성 안내로 연결하고, 13단계와 15단계는 각각 명령·협업형과 서술형 문제로 구분합니다.
