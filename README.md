...
웹 저장은 문제에 연결된 50개 파일만 허용합니다. 편집 중 다른 프로그램이 같은 파일을 바꾸면 이전 내용을 덮어쓰지 않고 충돌을 알리므로 `저장 내용 다시 불러오기`로 최신 파일을 확인한 뒤 다시 수정하세요. 웹에서 저장한 내용은 Git이 추적하는 실제 학습 파일에 반영됩니다. 11단계는 Spring 프로젝트 생성 안내로 연결하고, 13단계와 15단계는 각각 명령·협업형과 서술형 문제로 구분합니다.
